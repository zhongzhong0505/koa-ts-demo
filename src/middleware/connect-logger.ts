...
/**
   ...
   * @return {Array}
   */
function assembleTokens(req: any, res: any, customTokens: any) {
  const arrayUniqueTokens = (array: any) => {
  ...
  defaultTokens.push({
    token: /:req\[([^\]]+)]/g,
    replacement: function (_: any, field: any) {
...
/**
   ...
   * @api private
   */
function format(str: string, tokens: any) {
...
/**
   ...
   *         SAME AS "\\.jpg|\\.png|\\.gif"
   */
function createNoLogCondition(nolog: any) {
  let regexp = null;
...
/**
   ...
   * @api public
   */
export default function getLogger(logger4js: any, options: any) {
  ...
  return ({req, res}: {res:any, req: any}, next: any) => {
    ...
    if (thisLogger.isLevelEnabled(level) || options.level === 'auto') {
      ...
      res.writeHead = (code: any, headers: any) => {
